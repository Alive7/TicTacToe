...
int verifySquare(int* dex, struct Board* g, char* buff);
...
void readSquare(int* dex, struct Board* g, char* name);

// Ai functions
void AiMove(int* dex, struct Board* g);
...
void squareToDex(int* sq, int* dex) {
  // convert square number to index
...
void getName(struct Player* p) {
  ...
  while (strlen(buff) > NAME_SIZE + 1) {
    printf("Enter up to %d characters: ", NAME_SIZE);
    fgets(buff,BUFF_SIZE,stdin);
  }
  strncpy(p->name,buff,NAME_SIZE);
  removeNewLine(p->name);
  if (p->name[0] == '\0') strcpy(p->name,"com");
}
...
void printBoard(struct Board* g, struct Player* p1, struct Player* p2, int* init) {
  ...
  for (int r = 0; r < g->row; r++) {
    ...
    for (int c = 0; c < g->col; c++) {
      ...
      else printf("%c", g->grid[r*g->col+c]);
    }
